...
class DownToCloseImageView @JvmOverloads constructor(
        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (abs(deltaY) > touchSlop || abs(deltaX) > touchSlop) {
                    ...
                    lastX = event.rawX
                    velocityTracker?.addMovement(event)
                    updateBackgroundOpacity() // 更新背景透明度
                }
            }
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                if (imageView.translationY > closeThreshold || velocityY > 1000) {
                    ...
                    imageView.animate().translationY(0f)
                        ...
                        .setInterpolator(AccelerateDecelerateInterpolator())
                        .setDuration(300)
                        .withEndAction { backgroundAlpha = 1.0f; updateBackgroundOpacity() }
                        .start()
                }
            }
        }
    ...
    private fun updateBackgroundOpacity() {
        val distance = sqrt(imageView.translationY * imageView.translationY + imageView.translationX * imageView.translationX).coerceIn(0f, height.toFloat())
        backgroundAlpha = (1.0f - (distance / height).coerceIn(0f, 1f)) * transparencyMultiplier + 0.2f // 最小透明度為0.2
        val alphaInt = (backgroundAlpha * 255).toInt()
        setBackgroundColor((alphaInt shl 24) or 0x000000) // 根據移動距離變更背景透明度
    }
}